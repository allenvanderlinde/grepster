...
<li>The source code tries to maintain a standard but descriptive naming scheme for method variables whose scope does not leave the method in which it's declared. These variables usually will not have characters in their identifers that describe their datatype. Their names describe just what they are. For example, <code>wxButton button_ok</code>: <code>button_ok</code> will be found in each dialog's instantiation method which uses buttons, regardless of the kind of control or dialog box it's housed within.</li>
</ol>